...
$supportVpnScriptSource = Join-Path $SourceDir "playout-support-vpn.ps1"
if (-not (Test-Path -LiteralPath $supportVpnScriptSource -PathType Leaf)) {
  $supportVpnScriptSource = Join-Path $PSScriptRoot "playout-support-vpn.ps1"
}
if (-not (Test-Path -LiteralPath $supportVpnScriptSource -PathType Leaf)) {
  throw "Missing support VPN script: playout-support-vpn.ps1"
}
Copy-Item -LiteralPath $supportVpnScriptSource -Destination (Join-Path $InstallDir "playout-support-vpn.ps1") -Force

$supportVpnSource = Join-Path $SourceDir "support-vpn"
if (Test-Path -LiteralPath $supportVpnSource -PathType Container) {
  ...
  Copy-Item -LiteralPath (Join-Path $supportVpnSource "*") -Destination $supportVpnDestination -Recurse -Force
  $supportVpnNotice = Join-Path $SourceDir "SUPPORT-VPN-NOTICE.txt"
  if (Test-Path -LiteralPath $supportVpnNotice -PathType Leaf) {
    Copy-Item -LiteralPath $supportVpnNotice -Destination (Join-Path $InstallDir "SUPPORT-VPN-NOTICE.txt") -Force
  }
  try {
    & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $InstallDir "playout-support-vpn.ps1") `
      -Action ensure-service `
      -InstallDir $InstallDir `
      -PlayoutHome $PlayoutHome | Out-Null
  } catch {
    Write-Warning "Could not prepare Multivideo support VPN service: $($_.Exception.Message)"
  }
} else {
  Write-Warning "Support VPN payload not found in runtime package. Remote support VPN will be unavailable."
}
...
PLAYOUT_REMOTE_SUPPORT_LOGIN_SERVER=vpn.vorbio.me
