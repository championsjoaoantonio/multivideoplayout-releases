...
function Get-ChannelsForResume {
  ...
  foreach ($timeout in $timeouts) {
    try {
      return @(Invoke-RestMethod -Uri "$ApiBaseUrl/api/local/v1/channels" -UseBasicParsing -TimeoutSec $timeout)
    } catch {
      $lastError = $_.Exception.Message
      Write-UpdateLog "Could not list channels for resume with ${timeout}s timeout: $lastError"
      Start-Sleep -Seconds 1
    }
  }

  throw "Could not list channels for resume after retry: $lastError"
}
...
function Save-RunningChannelsForResume {
  ...
  try {
    $channels = Get-ChannelsForResume -ApiBaseUrl $apiBaseUrl
